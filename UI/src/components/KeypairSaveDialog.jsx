 ...
 *
 * Props (aligned with hooks_guide + RegistrationFlow usage):
 *   keypair    — raw keypair object from useRegister
 ...
 *   onConfirmed — () => void   called after user confirms download
 *                 (was incorrectly named onSaved in the original)
 */
export function KeypairSaveDialog({ keypair, publicKeys, onConfirmed }) {
  ...
  return (
    <div className="auth-root">
      <div className="keypair-box auth-card stack stack-16">
        {/* Header */}
        <div className="stack stack-4">
          ...
          <p className="text-muted" style={{ fontSize: "0.875rem" }}>
            This is the only time your private keys are available. There is no
            recovery — if you lose this file, received documents can no longer
            be decrypted. You can re-register, but old ciphertext will be
        ...
        {/* Confirmation — only shown after download */}
        {downloaded && (
          <label
            style={{
              display: "flex",
          ...
          >
            <input
              ...
              onChange={(e) => { if (e.target.checked) handleConfirm(); }}
            />
            I have saved the keypair file somewhere safe. I understand I cannot
            recover my private keys if I lose this file.
          </label>
        )}
...
/**
 * Serialise keypair to a JSON-safe bundle.
 * Keys are stored as hex for human-readability and round-trip safety.
 * Version field + username make the file self-describing for VaultUnlock parser.
 */
