...
class Transceiver:
    ...
    def upload(
        ...
        Derives caller's public key from the supplied private key.
        Prefer upload_with_public_key() when the public key is already known
        (e.g. fetched from the DB via JWT) to avoid any key-mismatch issues.
        """
        pub_hex, pub_hash = self._caller_keys(caller_private_key_pem)
        return self._do_upload(pub_hex, pub_hash, plaintext, filename, tags)

    def upload_with_public_key(
        self,
        caller_public_key_hex: str,
        caller_public_key_hash: str,
        plaintext: bytes,
        filename: str,
        tags: list[str] | None = None,
    ) -> UploadResult:
        """
        Encrypt *plaintext* and store it using the caller's *public* key directly.

        This is the preferred upload path for the web/API layer:
          - The JWT / require_auth dependency already carries the DB-stored
            public_key_hex and public_key_hash for the authenticated user.
          - Using those values guarantees owner_key_hash == public_key_hash
            in list_records — no PEM re-derivation, no mismatch possible.
          - The private key never has to leave the client.

        Flow:
          1. Accept caller's public key (hex-encoded uncompressed point).
        ...
        """
        return self._do_upload(caller_public_key_hex, caller_public_key_hash,
                               plaintext, filename, tags)
    ...
    def _do_upload(
        ...
        pub_hex, pub_hash = pub_hex, pub_hash  # already resolved by caller
